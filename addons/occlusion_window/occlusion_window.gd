...
class_name OcclusionWindow
extends Node2D

## Reveal-window component for the Lit lighting addon. Mount one as a child of the 2D
## camera and/or on the character: each punches a soft-edged circular window through
## every Lit receiver that opts in (lit_window_occlude = true on its material) and is
## drawn IN FRONT of the target's y-sort depth line. A character hidden by a tile layer,
## building or tree stays visible through the hole, with a vignette fading back to the
## occluder.
##
## The window is a screen-space effect: the Lit receiver shader reads a set of global
## uniforms (the lit_window_a_* / lit_window_b_* slots in lit_header.gdshaderinc and
## lit_fragment.gdshaderinc) and fades its alpha out at the target's SCREEN position.
## Because the cut is per fragment and screen-based it applies to sprites, plain nodes
## AND TileMapLayer / LitTileMapLayer tiles alike, and works on both the Forward+ and
## the OpenGL/Compatibility (web) renderers. This addon depends on the `addons/lit`
## receiver pipeline being present.
##
...
## lit_window_occlude left off to stay untouched.
...
var _prefix := "lit_window_a_"


func _process(_delta: float) -> void:
	if Engine.is_editor_hint():
		return
	_prefix = "lit_window_b_" if window_slot == 1 else "lit_window_a_"
	_update()

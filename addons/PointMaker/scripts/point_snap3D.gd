...
func _ready():
	...
	if not has_node("CollisionShape3D") and not has_node("CollisionPolygon3D"):
		push_warning("⚠ PointSnap3D requires a CollisionShape3D or CollisionPolygon3D to define the snap zone.")

func _on_body_entered(body: Node):
	if not body or not body.is_class("CharacterBody3D"):
		return
